...
-- Precise triangle vs AABB intersection test using the Separating Axis Theorem (SAT).
-- Covers all cases: triangle inside box, box inside triangle, and partial overlap.
-- @param triangle  table  { P1, P2, P3 } where each field is a Vector3
-- @return          boolean
function BoundBox:IntersectTriangle(triangle)
    local p1, p2, p3 = triangle.P1, triangle.P2, triangle.P3

    -- 1. AABB of the triangle vs this box (fast early-out)
    local tMinX = math.min(p1.x, p2.x, p3.x)
    local tMaxX = math.max(p1.x, p2.x, p3.x)
    local tMinY = math.min(p1.y, p2.y, p3.y)
    local tMaxY = math.max(p1.y, p2.y, p3.y)
    local tMinZ = math.min(p1.z, p2.z, p3.z)
    local tMaxZ = math.max(p1.z, p2.z, p3.z)

    -- log('aaaaaaaaaaaaa', tMinX, tMaxX, tMinX, tMinY, tMaxY, tMinZ, tMaxZ)
    -- log('bbbbbbbbbbbbb', self.min.x , self.max.x, self.min.y, self.max.y, self.min.z, self.max.z)
    -- log()
    if tMaxX < self.min.x or tMinX > self.max.x then return false end
    if tMaxY < self.min.y or tMinY > self.max.y then return false end
    if tMaxZ < self.min.z or tMinZ > self.max.z then return false end

    -- Helper: project three triangle vertices and the box onto an axis,
    -- return true if the intervals are separated.
    local cx = (self.min.x + self.max.x) * 0.5
    local cy = (self.min.y + self.max.y) * 0.5
    local cz = (self.min.z + self.max.z) * 0.5
    local ex = (self.max.x - self.min.x) * 0.5
    local ey = (self.max.y - self.min.y) * 0.5
    local ez = (self.max.z - self.min.z) * 0.5

    -- Translate triangle so box center is at origin
    local v0x, v0y, v0z = p1.x - cx, p1.y - cy, p1.z - cz
    local v1x, v1y, v1z = p2.x - cx, p2.y - cy, p2.z - cz
    local v2x, v2y, v2z = p3.x - cx, p3.y - cy, p3.z - cz

    -- Triangle edges
    local e0x, e0y, e0z = v1x - v0x, v1y - v0y, v1z - v0z
    local e1x, e1y, e1z = v2x - v1x, v2y - v1y, v2z - v1z
    local e2x, e2y, e2z = v0x - v2x, v0y - v2y, v0z - v2z

    -- SAT test on a given axis (ax, ay, az)
    local function testAxis(ax, ay, az)
        local p0 = ax * v0x + ay * v0y + az * v0z
        local p1_ = ax * v1x + ay * v1y + az * v1z
        local p2_ = ax * v2x + ay * v2y + az * v2z
        local pMin = math.min(p0, p1_, p2_)
        local pMax = math.max(p0, p1_, p2_)
        local r = ex * math.abs(ax) + ey * math.abs(ay) + ez * math.abs(az)
        return pMin > r or pMax < -r  -- true means separated
    end

    -- 2. Nine cross-product axes: edge x box-face-normal
    -- a00 = e0 x (1,0,0)
    if testAxis(0, -e0z, e0y)  then return false end
    -- a01 = e0 x (0,1,0)
    if testAxis(e0z, 0, -e0x)  then return false end
    -- a02 = e0 x (0,0,1)
    if testAxis(-e0y, e0x, 0)  then return false end
    -- a10 = e1 x (1,0,0)
    if testAxis(0, -e1z, e1y)  then return false end
    -- a11 = e1 x (0,1,0)
    if testAxis(e1z, 0, -e1x)  then return false end
    -- a12 = e1 x (0,0,1)
    if testAxis(-e1y, e1x, 0)  then return false end
    -- a20 = e2 x (1,0,0)
    if testAxis(0, -e2z, e2y)  then return false end
    -- a21 = e2 x (0,1,0)
    if testAxis(e2z, 0, -e2x)  then return false end
    -- a22 = e2 x (0,0,1)
    if testAxis(-e2y, e2x, 0)  then return false end

    -- 3. Triangle face normal axis
    local nx = e0y * e1z - e0z * e1y
    local ny = e0z * e1x - e0x * e1z
    local nz = e0x * e1y - e0y * e1x
    if testAxis(nx, ny, nz) then return false end

    -- No separating axis found -> intersecting or contained
    return true
end
