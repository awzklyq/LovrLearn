...
function Matrix2D:CreateLove2dMatrix()
    if not self._love2d_transform then
        self._love2d_transform = love.math.newTransform()
    end
...
function Matrix2D:unuse()
    ...
    local _NewMat =  self:Copy()
    if _NewMat:Inverse() then
        _NewMat:CreateLove2dMatrix()
        love.graphics.applyTransform(_NewMat._love2d_transform);
    end
end
    ...
    --         self[6] = vx; self[7] = vy; 
    --     end
    Matrix2D.cIdentity = Matrix2D.Identity()