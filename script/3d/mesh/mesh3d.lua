...
function Mesh3D:SetImage(image)
    if type(image) == "string" then
        self._BaseImage = ImageEx.new(image)
    else
        self._BaseImage = image
    end

    self:setTexture(self._BaseImage.obj)
end
...
-- Check whether any MeshTriangle of this mesh is inside the given BoundBox
-- @param box  BoundBox  the bounding box to test against
-- @return     boolean   true if at least one MeshTriangle is inside the box
function Mesh3D:HasMeshTriangleInBox(box)
     self.FacesInfos[#self.FacesInfos + 1] = FaceInfo
    for i = 1, #self.FacesInfos do
        local f = self.FacesInfos[i]
        if box:IntersectTriangle(f.Triangle) then
            return true
        end
    end
    return false
